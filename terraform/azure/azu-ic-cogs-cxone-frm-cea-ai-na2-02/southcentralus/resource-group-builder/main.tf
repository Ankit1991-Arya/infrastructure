...
resource "azurerm_role_assignment" "Default_OpenAISPCEA_Developers" {
  scope                = "/subscriptions/${data.azurerm_subscription.this.subscription_id}"
  role_definition_name = "Cognitive Services OpenAI Contributor"
  principal_id         = "1b781f36-73c9-4a84-b2db-4fb7bf337c08"
}

resource "azurerm_role_assignment" "Default_ContributorSPCEA_Developers" {
  scope                = "/subscriptions/${data.azurerm_subscription.this.subscription_id}"
  role_definition_name = "Contributor"
  principal_id         = "1b781f36-73c9-4a84-b2db-4fb7bf337c08"
}

resource "azurerm_role_assignment" "Default_OpenAISPAutoSummary_Developers" {
  scope                = "/subscriptions/${data.azurerm_subscription.this.subscription_id}"
  role_definition_name = "Cognitive Services OpenAI User"
  principal_id         = "8ee4c808-0c6e-43ff-aaaa-2b5bc5c15310"
}

resource "azurerm_role_assignment" "Default_MonitoringSPGrafana_Developers" {
  scope                = "/subscriptions/${data.azurerm_subscription.this.subscription_id}"
  role_definition_name = "Monitoring Reader"
  principal_id         = "f9ae4066-69af-4444-bd22-e090988e81a3"
}
