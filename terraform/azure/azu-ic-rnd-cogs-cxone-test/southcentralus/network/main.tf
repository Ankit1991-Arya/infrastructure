...
locals {
  ...
  zone_id            = "Z17NXXZOR4E7VM"
  resource_group_net = "networkinfra-tt"
  resource_group_sec = "networksecurity-tt"
  location           = "South Central US"
  key_vault_id       = "/subscriptions/56a46408-b4a2-45df-aef5-92cddf4ef946/resourceGroups/regionconfig-tt/providers/Microsoft.KeyVault/vaults/ic-envvar-tt"
  sku                = "Standard"
  #  sku_v2             = "Standard_v2"
  allocation_method = "Static"
  system_vault_id   = "/subscriptions/56a46408-b4a2-45df-aef5-92cddf4ef946/resourceGroups/regionconfig-tt/providers/Microsoft.KeyVault/vaults/ic-systems-tt"
  #  network_vault_id   = "/subscriptions/56a46408-b4a2-45df-aef5-92cddf4ef946/resourceGroups/networkinfra-tt/providers/Microsoft.KeyVault/vaults/ic-network-tt"
...
module "agw_regional_app" {
  ...
  app_gate_name = "regional-service-app-agw-tt700"
...
module "agw_cluster_app_agw" {
  depends_on    = [module.cert_assigned_identity_agw]
  source        = "../../../../../modules/azure-network-alb"
  app_gate_name = "cluster-app-agw-tt700"
  # sku_name                       =  ["Standard_v2" ,"Standard_v2" ]
  sku_name                       = ["WAF_v2", "WAF_v2"]
  firewall_policy_id             = module.waf_for_cluster.firewall_policy_id
  location                       = local.location
  ...
  listener_list = [
    ...
    #incontrol-tt700.nice-incontact.com
    {
...
#locals {
#  migration_cluster-id            = "tt700"
#  migration_pool_storagemigration = "ng-storagemigration-backend-pool"
#  migration_probe_name_list       = ["targetgroupstoragemigration"]
#  migration_listener_name_list    = ["StorageMigration"]
#  migration_port_name             = "port-3333"
#  migration_protocol_http         = "Http"
#}
...
#  frontend_ip_configuration_name = "pa-lb-frontend"
#  subnet_id                      = azurerm_subnet.Serverless-subnet.id
#  lb_name                        = "tt99-storagemigration-lb"
#  sku                            = "Standard"
#  backend_address_pool_name      = "tt99-storagemigration-pool"
#  prob_list                      = [
#                                  {
#                                    probe_name = "tt99-storagemigration-probe"
#                                    probe_protocol =  "Http"
#                                    probe_port   =  8888
#                                    probe_interval_in_seconds = 5
#                                    probe_request_path  = "/"
...
#  rule_list =  [
#                                  {
#                                    rule_name = "tt99-storagemigration-rule"
#                                    rule_protocol =  "Tcp"
...
#module "storagemigration_agw" {
#  depends_on                     = [module.cert_assigned_identity_agw]
#  source                         = "../../../../../modules/azure-network-alb"
#  app_gate_name                  = "storage-agw"
#  sku_name                       =  ["Standard_v2" ,"Standard_v2" ]
#  location                       = local.location
#  resource_group                 = local.resource_group_net
#  subnet_id                      = azurerm_subnet.Serverless-subnet.id
#  gateway_ip_configuration_name  = "storage-gateway-ip-configuration"
#  frontend_ip_configuration_name = "storage-agw-ip-conf"
#  pip_name                       = "storage-agw-pip-2"
#  backend_address_pool_name_list = [
#    {
#      name = local.migration_pool_storagemigration
#    },
#  ]
#  probe_name_list = [
#    {
#      name     = local.migration_probe_name_list [0]
#      path     = "/"
#      protocol = "Http"
#      port     = 8888
#      backend_settings_port = 3333
#    },
#  ]
#  listener_list = [
#    {
#      listener_name = format("listener-http-%s", local.migration_listener_name_list[0])
#      port_name     = local.migration_port_name
#      protocol      = local.migration_protocol_http
#      host_name     = format("storagemigration-%s.%s", local.migration_cluster-id , data.azurerm_key_vault_secret.external-domain-name.value)
#    },
#  ]
#  request_routing_rule_set = [
#{
#      name                  = format("http-%s", local.migration_listener_name_list[0])
#      pool_name             = local.migration_pool_storagemigration
#      target_listener_name  = null
#      listener_name         = format("listener-http-%s", local.migration_listener_name_list[0])
#      backend_http_settings = local.migration_probe_name_list[0]
#      redirect              = false
#      redirect_conf_name    = null
#    },
#  ]
...
locals {
  ...
  # TT700.nice-incontact.com
  #
  # api-tt700.test.nice-incontact.com
  #tt700.test.nice-incontact.com
  #home-tt700.nice-incontact.com
  #incontrol-tt700.test.nice-incontact.com
}
...
module "waf_for_cluster" {
  ...
  location       = local.location
  resource_group = local.resource_group_net
  wafpolicy_name = "waf-for-cluster-policy-test"
}
########
#########pa
#pa update v2.2
...
module "palo_alto" {
  ...
  mgmnt-int-name                   = "mgmnt-int"
  trust-int-name                   = "trust-int"
  untrust-int-name                 = "untrust-int"
