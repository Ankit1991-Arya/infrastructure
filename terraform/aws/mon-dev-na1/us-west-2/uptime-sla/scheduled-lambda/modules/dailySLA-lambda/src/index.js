...
async function modifyDailySLA(key, jsonData) {
  ...
  console.log('response modifyDailySLA', response);
  return response;
}
...
async function constructDailySLA(jsond) {
  const jsonData = { [jsond.day]: jsond.sla };
  ...
  try {
    ...
    console.log('dailySLA', dailySLA);
    const updatedSLA = { ...dailySLA, ...jsonData };
    await modifyDailySLA(key, updatedSLA);
    return dailySLA;
  } catch (error) {
    if (error.name === 'NotFound') {
      return await modifyDailySLA(key, jsonData);
    }
...
exports.handler = async () => {
  try {
    ...
    for (const service of services) {
      const { year, month, day } = constructKey(1);
      ...
      try {
        ...
        const sla = dailyUptimeSLA(JSON.parse(data), service?.monitoring_frequency || 5);
        console.log('sla', sla);
        const slaDetails = {
        ...
        const created = await constructDailySLA(slaDetails);
        console.log('DailySLA created successfully', created);
      } catch (error) {
        if (error.name === 'NotFound') {
          ...
          console.log('slaDetails with zero', slaDetails);
          const created = await constructDailySLA(slaDetails);
          console.log('DailySLA created successfully', created);
        } else {
