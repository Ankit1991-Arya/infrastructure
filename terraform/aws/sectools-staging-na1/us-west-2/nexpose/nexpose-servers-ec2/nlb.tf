locals {
  lb_name = "NexposePOC-Console-NLB"
}
...
resource "aws_lb" "nexpose_private_nlb" {
  ...
  security_groups            = [module.nlb_aws_security_group["nexpose-console-fips"].sg_id]
...
resource "aws_lb_target_group_attachment" "nlb_console_tg_attachment" {
  ...
  target_id = module.nexpose_servers.instance_id["nexpose-console-fips"]
