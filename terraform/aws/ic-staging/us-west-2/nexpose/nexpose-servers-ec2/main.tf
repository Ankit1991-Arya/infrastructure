...
module "corenetwork_privatelink_endpoint" {
  ...
  #  version = "0.1.2"
  source = "github.com/inContact/aws-terraform-modules-network.git//modules/aws_privatelink_vpc_endpoint?ref=add-lattice-endpoint"
  private_link_data = { for k, a in local.private_link_data.private_link_data_corenetwork : k =>
    merge(a, { security_group_ids = lookup({ nexpose-console-corenetwork = [module.vcpe_aws_security_group["nexpose-console-endpoint-sg"].sg_id] }, k, a.security_group_ids)
  }) }
  depends_on = [module.vcpe_aws_security_group]
}

module "vcpe_aws_security_group" { # used for vpc_security_group_ids, need to set output to sg_id in security_groups module to use here
