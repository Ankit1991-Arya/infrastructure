  ...
  policy = <<EOF
{
  ...
  "Statement": [
    ...
    },
    {
      ...
      "Action": "sns:Publish",
      "Resource": "arn:aws:sns:us-west-2:032474939542:shared_eks01_infra_notifications"
    }
  ]
}
EOF
...
resource "aws_sns_topic_subscription" "email_subscription_br" {
  ...
  protocol  = "email"
  endpoint  = "bhagyesh.raut@nice.com" # Replace with your email
}

resource "aws_sns_topic_subscription" "email_subscription_sl" {
  topic_arn = aws_sns_topic.shared_eks01_infra_notifications.arn
  protocol  = "email"
  endpoint  = "shrikant.lavhate@nice.com" # Replace with your email
}
