...
module "state-streams-em-contact" {
  ...
  schema-registry-url                      = "https://schema-registry-ss-vpce-eu1.omnichannel.prod.internal/schema-registry"
  schema-registry-mock                     = false
  schema-registry-auto-register-schemas    = false

  snapshots-enabled                       = false
  region                                  = "eu-central-1"
  environment-prefix                      = "af"
  security-groups                         = ["sg-0de8069fd2bf9faee"]
  subnets                                 = ["subnet-084c9f807729eb262", "subnet-0e63f59f7b8d8df8a", "subnet-0da252180a730d680"]
  build-artifacts-s3-bucket               = "af-entitymanagement"
  alarm-sns-topic-name                    = "entitymanagement_alerts_account_prod"
  parallelism                             = 4
  parallelism-per-kpu                     = 1
  schema-registry-ssl-cert-s3-bucket-name = "ic-certs-af"
...
module "state-streams-dfo-contact" {
  ...
  kinesis-stream-names       = "af-de-case-events-stream" // All the stream names that the flink app will subscribe to
  deploy-test-kinesis-stream = true
  kafka-output-topic         = "SuiteData.ContactEvent.V2"
  kafka-test-output-topic    = "SuiteData.IntegrationTests.ContactEvent.V1"
...
module "state-streams-dfo-agent-contact" {
  ...
  input-source               = "kinesis"
  kinesis-stream-names       = "prod-agent-session,af-de-platform-events-stream,af-de-case-events-stream"
  deploy-test-kinesis-stream = true
