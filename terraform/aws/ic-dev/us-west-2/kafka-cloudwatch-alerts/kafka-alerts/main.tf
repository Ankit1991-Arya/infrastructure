...
variable "sns_topic_info" {
  type    = string
  default = "arn:aws:sns:us-west-2:300813158921:cloudwatch-bmc-minor"
}

variable "sns_topic_critical" {
  type    = string
  default = "arn:aws:sns:us-west-2:300813158921:cloudwatch-bmc-critical"
}
...
resource "aws_cloudwatch_metric_alarm" "kafka_connection_count_info" {
  ...
  alarm_name          = "DEV-KAFKA-ConnectionCount-Broker-${each.value}-MINOR"
...
resource "aws_cloudwatch_metric_alarm" "kafka_connection_count_critical" {
  for_each            = toset(var.broker_ids)
  alarm_name          = "DEV-KAFKA-ConnectionCount-Broker-${each.value}-CRITICAL"
  comparison_operator = "GreaterThanThreshold"
  evaluation_periods  = 1
  threshold           = 4000
