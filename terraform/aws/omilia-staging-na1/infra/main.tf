...
module "omilia" {
  source = "github.com/inContact/omilia-terraform-modules?ref=2.0.6.0"
  ...
  public_subnet_cidrs        = ["10.254.0.0/24", "10.254.1.0/24", "10.254.2.0/24", "52.25.242.82/32", "54.190.209.154/32", "54.186.246.199/32", "66.35.27.114/32", "190.181.36.252/32", "190.181.6.124/32", "190.181.6.123/32", "190.181.36.239/32", "66.103.20.137/32", "66.35.27.214/32", "136.38.113.166/32", "67.214.231.34/32"]
  private_subnet_cidrs       = ["10.254.3.0/24", "10.254.4.0/24", "10.254.5.0/24"]
  database_subnet_cidrs      = ["10.254.6.0/24", "10.254.7.0/24", "10.254.8.0/24"]
  public_subnet_ids          = ["subnet-0527c614b8d0e4ce5", "subnet-0b93712811ec30ae3", "subnet-07908d6add26fa251"]
  private_subnet_ids         = ["subnet-05a1fbbb1bb8edaf6", "subnet-0644b481d3f1edc44", "subnet-0bcab1ed20b93b61b"]
  database_subnet_ids        = ["subnet-04979dcd86b915a3a", "subnet-0e3f68bfeb5cf17a1", "subnet-06a541e037e24b854"]
  private_nat_eip_public_ips = ["52.25.242.82/32", "54.186.246.199/32", "54.190.209.154/32"]
  private_route_table_ids    = ["rtb-08a20c2cb75af3a3c", "rtb-04a0d5e7c1048e6fc", "rtb-0f14dba4741720e93"]
  ...
  cluster_endpoint_public_access_cidrs = concat([
    ...
    "54.186.246.199/32",
    "190.181.37.247/32"
  ], local.temporary_access_ips)

  external_access_cidrs = concat([
    "66.35.27.114/32",
    "190.181.36.252/32",
    "190.181.6.123/32",
    "190.181.6.124/32",
  ...
  map_roles = [
    {
      ...
      "rolearn" : "arn:aws:iam::339713029084:role/GroupAccess-DevOpsAdmin",
      "username" : "GroupAccess-DevOpsAdmin"
    },
    {
      "groups" : ["system:masters"],
      "rolearn" : "arn:aws:iam::339713029084:role/GroupAccess-SRE",
  ...
  ssm_scm_user_token            = "/github/prod/users/ntmggr"
  ...
  slack_webhook              = ""
  cloudwatch_email_recipient = null
  ...
  prod_peers = ["twilio", "omilia_pbx", "lab"]

  kamailio_asg_desired_instance_size  = 3
  rtpengine_asg_desired_instance_size = 3

  certificate_name_crt = "ocp-stage/kamailio/cert"
  certificate_name_key = "ocp-stage/kamailio/cert_key"

  kamailio_ami_id               = "ami-0e9f59ceae443ec28" # Includes Rapid7 agent
  rtpengine_ami_id              = "ami-0e9f59ceae443ec28" # Includes Rapid7 agent
  kamailio_additional_policies  = ["arn:aws:iam::339713029084:policy/Rapid7_TokenAccess"]
  rtpengine_additional_policies = ["arn:aws:iam::339713029084:policy/Rapid7_TokenAccess"]

  #======================
  # Custom options
  #======================
  standard_tags               = local.standard_tags
  deployment_principal_arn    = "arn:aws:iam::339713029084:role/ServiceAccess-infrastructure-live-terraform"
  disable_slack_webhook       = true
  cluster_manage_aws_auth     = false

  # Auth
  argocd_enable_github_login = false
  ...
  alarm_rename_rules = [
    {
      ...
      "new_name" = "Omilia-Staging-ocp-opensearch[AWS/ElastiCache=>CurrConnections]"
    },
    {
      "pattern"  = "Endpoint(.+)",
      "new_name" = "Omilia-Staging-ocp-endpoints[AWS/EKS=>Endpoint=>\\1]"
    },
    {
      "pattern"  = ".+",
      "new_name" = "Omilia-Staging-ocp-endpoints[None]"
    }
  ]
  ...
  # ArgoCD
  argocd_credentials_templates = {
    https-creds3 = {
      url      = "https://github.com/inContactBuildUser"
      username = "inContactBuildUser"
      password = data.aws_ssm_parameter.build_user_token.value
    }
  }
}

data "aws_ssm_parameter" "build_user_token" {
  name = "/github/prod/users/inContactBuildUser"
}
