...
module "omilia_privatelink_endpoint" {
  ...
  private_link_data = { for k, a in local.private_link_data.private_link_data : k =>
    merge(a, { security_group_ids = lookup({ snowflake-omilia = [module.endpoint_aws_security_group["snowflake-omilia-vpce-sg"].sg_id] }, k, a.security_group_ids)
  }) }
