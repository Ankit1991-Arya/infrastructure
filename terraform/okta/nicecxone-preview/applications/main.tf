...
provider "aws" {
  alias = "ic_ops"
  assume_role {
    role_arn = "arn:aws:iam::196965494462:role/ServiceAccess-infrastructure-live-terraform"
  }
  region = "us-west-2"
}

resource "aws_iam_saml_provider" "oktapreview_ic_ops" {
  provider               = aws.ic_ops
