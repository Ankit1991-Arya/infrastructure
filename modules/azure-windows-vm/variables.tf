...
variable "virtual_machine_computer_name" {
  ...
  validation {
    condition     = length(var.virtual_machine_computer_name) <= 15
    error_message = "Windows Virtual Machine computer/host names must be 15 or fewer characters."
  }
}
...
variable "license_type" {
  description = "Specify which type of Azure Hybrid Use Benefit license to be used."
  type        = string
  default     = "None"

  validation {
    condition     = contains(["None", "Windows_Client", "Windows_Server"], var.license_type)
    error_message = "Valid values are 'None', 'Windows_Client', or 'Windows_Server'."
  }
}
...
variable "patch_mode" {
  ...
  validation {
    ...
    error_message = "Valid values are 'Manual', 'AutomaticByOS', or 'AutomaticByPlatform'."
  }
}
...
# Active Directory domain join extension variables
variable "ad_domain_name" {
  description = "Name of Active Directory domain to join. Leaving this blank will skip implementation of the AD domain join extension."
  type        = string
}
variable "ad_ou_path" {
  description = "Specify organizational unit (OU) path for Active Directory domain join."
  type        = string
  default     = ""
}
variable "ad_domain_join_username" {
  description = "Username to use when joining Active Directory domain."
  type        = string
  # default     = ""
}
variable "ad_domain_join_password" {
  description = "Password to use when joining Active Directory domain."
  type        = string
  sensitive   = true
}
...
variable "disk_initialization_dsc_extension" {
  description = "Enable implementation of disk initialization Powershell DSC VM extension."
  type        = bool
  default     = false
}
variable "tags" {
